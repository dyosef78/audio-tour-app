...
import type { DynamicRouteRequest, DynamicRouteResult } from '../../routing/routeDecision';
 ...
 *
 * !! THE SERVER SIDE OF THIS DOES NOT EXIST YET. !!
 *
 * This is written against the contract below. Until a function with this name
 * is deployed, every call returns `unavailable` (HTTP 404) and the app keeps
 * the bundled route - which is the designed fallback, so shipping the client
 * first is safe.
 *
 ...
 *   404   function not deployed        -> unavailable, not retried
 *   501   routing not configured       -> unavailable, not retried
 *   other                              -> failed, retried with backoff
 *
